...
from .generators import building, bridge, road, highrise, tunnel, generic  # Updated imports
from bimflow.consumers import broadcast_progress  # WebSocket
import logging
...
def generate_ifc_task(self, model_id, asset_type_code, spec_json, scenario_id=None):
    try:
        ...
        if not gen_func:
            logger.warning(f"Using generic fallback for {asset_type_code}; implement custom generator.")
        ...
        if task_instance:
            task_instance.mark_completed({'status': 'success', 'model_id': model_id, 'fallback_used': spec_json.get('_fallback_used', False)})
